...
DEFAULT_PHASE_FIGURE_PATH = PROJECT_ROOT / "figures" / "fig2" / "smoke_fig2_transmission_phase.png"

if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from figures.fig2.plot_fig2 import load_phase_response, load_spectrum, plot_transmission, plot_transmission_phase  # noqa: E402
from geometry.fig2.geometry import default_diameter_nm  # noqa: E402
from scripts.run_fig2 import run_sweep_with_phase, save_phase_response, save_spectrum  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--phase-figure-path", type=Path, default=DEFAULT_PHASE_FIGURE_PATH)
    parser.add_argument("--output-stem", default="smoke_fig2_spectrum")
    parser.add_argument("--phase-output-stem", default="smoke_fig2_phase")
...
def main() -> None:
    ...
    loaded_phase = load_phase_response(phase_npy_path)
    if loaded_phase.shape != (args.num_wavelengths, 5):
        raise AssertionError(f"Unexpected phase response shape: {loaded_phase.shape}")
    if not np.all(np.isfinite(loaded_phase)):
        raise AssertionError("Phase response contains non-finite values")

    plot_transmission(loaded, args.figure_path)
    plot_transmission_phase(loaded_phase, args.phase_figure_path)
    for path in (npy_path, csv_path, phase_npy_path, phase_csv_path, args.figure_path, args.phase_figure_path):
    ...
    print(f"  phase_npy: {phase_npy_path}")
    print(f"  phase_csv: {phase_csv_path}")
    print(f"  figure: {args.figure_path}")
    print(f"  phase_figure: {args.phase_figure_path}")
